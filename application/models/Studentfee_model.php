...
class Studentfee_model extends CI_Model
{
    ...
    public function remove( $id, $sub_invoice )
    {
        ...
        if ( $q->num_rows() > 0 ) {
            ...
            $_amount = ( intval( $b[$sub_invoice]['amount'] ) + intval( $b[$sub_invoice]['amount_fine'] ) ) - intval( $b[$sub_invoice]['amount_discount'] );
            $_amount = 0 - $_amount;
            // adding record of the student fee revert
            $this->transaction_model->addTransaction( "Student fee revert", $_amount );
        }
    }
