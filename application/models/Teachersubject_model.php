...
class Teachersubject_model extends CI_Model
{
    ...
    public function get( $id = null, $teacher_id = null )
    {
    ...
    public function getDetailbyClsandSection( $class_id, $section_id, $exam_id )
    {
        ...
        $query = $this->db->query( "SELECT teacher_subjects.*,exam_schedules.date_of_exam,exam_schedules.start_to,
exam_schedules.end_from,exam_schedules.room_no,teacher_subjects.max_marks,teacher_subjects.passing_marks,subjects.name,
    ...
    public function getSubjectByClsandSectionTeacher( $class_id, $section_id ,$teacher_id )
    {
        $query = $this->db->query( "SELECT teacher_subjects.*,teachers.name as `teacher_name`, subjects.name,subjects.type FROM `teacher_subjects` INNER JOIN subjects ON teacher_subjects.subject_id = subjects.id INNER JOIN class_sections ON teacher_subjects.class_section_id = class_sections.id INNER JOIN teachers ON teachers.id = teacher_subjects.teacher_id  WHERE class_sections.class_id =" . $this->db->escape( $class_id ) . " and class_sections.section_id=" . $this->db->escape( $section_id ) ." and teacher_subjects.teacher_id=" . $this->db->escape( $teacher_id ) . " and teacher_subjects.session_id=" . $this->db->escape( $this->current_session ) );
        return $query->result_array();
    }
    ...
    function getTeacherClassSubjects( $teacher_id )
    {
	  ...
	  function getTeacherClassSubjects2( $teacher_id )
    {
        $this->db->select( 'teacher_subjects.*,subjects.name,classes.class,classes.id as class_id,sections.section,sections.id as section_id' );
        $this->db->from( 'teacher_subjects' );
        $this->db->join( 'subjects', 'subjects.id = teacher_subjects.subject_id' );
        $this->db->join( 'class_sections', 'class_sections.id = teacher_subjects.class_section_id' );
        $this->db->join( 'classes', 'classes.id = class_sections.class_id' );
