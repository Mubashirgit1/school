...
class Expense extends Admin_Controller
{
    ...
    function index()
    {
       ...
       $this->form_validation->set_rules( 'exp_head_id', 'Expense Head', 'trim|required|xss_clean' );
         $this->form_validation->set_rules( 'amount_expense', 'Amount', 'trim|required|xss_clean' );
        ...
        if ( $this->form_validation->run() == FALSE ) {
            ...
            $data = array(
				...
				'amount' => $this->input->post_get( 'amount_expense' ),
                'note' => $this->input->post_get( 'description' ),
				'user_id' => $this->input->post_get( 'admin' )
            );
        ...
        if( $search == "search_filter" ){
            ...
            $expense_result       = $this->expense_model->search( "", $date_from, $date_to, $expense_head_name );
	...
	function edit2($id){
        ...
        if ( $this->form_validation->run() == FALSE ) {
		  ...
		  $date = date("m/d/Y", now()); 
            ...
            redirect( 'transactions/daily'."?date_from=" . urlencode( "$date" ) . "&date_to=" . urlencode( "$date" ) );
        }
