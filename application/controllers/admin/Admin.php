...
class admin extends Admin_Controller
{
    ...
    function dashboard()
    {
        ...
        $total_receiveable_fee = $this->student_fee_payments->get_total_receiveable_fee_per_month();
        $data['total_receiveable_fee'] = $total_receiveable_fee;

        $total_fee_received_fee = $this->student_fee_payments->get_total_received_fee_per_month( date( 'Y-m-d', now() ) );
        $data['total_fee_received_fee'] = $total_fee_received_fee;

        $other_fee_received_sum = $this->student_fee_payments_other->sum_by_month( date( 'Y-m-d', now() ) );
        $total_fee_including_other_fee = $other_fee_received_sum + $total_fee_received_fee;
        $data['total_fee_including_other_fee'] = $total_fee_including_other_fee;

        $sum_total_due_fee = $this->student_model->sum_total_due_fee();
        $data['sum_total_due_fee'] = $sum_total_due_fee;

        $sum_teacher_salary = $this->teacher_model->sum_teachers_salary();

        $sum_payable_teacher_salaries = $sum_teacher_salary['teacher_salary_sum'];
        $data['sum_payable_teacher_salaries'] = $sum_payable_teacher_salaries;

        $sum_due_teacher_salaries = $sum_teacher_salary['due_salary_sum'];
        $data['sum_due_teacher_salaries'] = $sum_due_teacher_salaries;

        $sum_paid_teacher_salaries = $this->teacher_salary_payment->sum_paid_by_month( date( 'Y-m-d', now() ) );
        $data['sum_paid_teacher_salaries'] = $sum_paid_teacher_salaries;

        $calculate_attendance_by_date = $this->stuattendence_model->calculate_attendance_by_date( date( 'Y-m-d', now() ) );
        $data['calculate_attendance_by_date'] = $calculate_attendance_by_date;
        ...
        $total_books = $this->book_model->calculate_total_books();
        $data['total_books'] = $total_books;

        $books_issued = $this->bookissue_model->calculate_issued_books();
        $data['books_issued'] = $books_issued;

        $total_hostel_rooms = $this->hostelroom_model->calculate_total_rooms();
        $data['total_hostel_rooms'] = $total_hostel_rooms;
        ...
        $sum_staff_salary = $this->staff_model->sum_staff_salary();
        $data['sum_staff_salary'] = $sum_staff_salary;

        $sum_staff_salary_paid = $this->staff_salary_payments_model->sum_paid_by_month( date( 'Y-m-d', now() ) );
        $data['sum_staff_salary_paid'] = $sum_staff_salary_paid;
