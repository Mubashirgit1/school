...
<div class="content-wrapper" style="min-height: 946px;">
    ...
    <section class="content">
                            ...
                            <div class="form-group">
                                ...
                                <select id="class_id" name="class_id" class="form-control">
                                    ...
                                    foreach ( $classlist as $class ) {
                                        ?>
                                        <option value="<?php echo $class['id'] ?>" <?= ( $input_class_id == $class['id'] ? 'selected="selected"' : '' ) ?>><?php echo $class['class'] ?></option>
                                        <?php
                                        $count++;
                                    }
                                    ?>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select id="section_id" name="section_id" class="form-control">
                                    <option value="<?= ( !empty( $input_section_id ) ? $input_section_id : "" ) ?>"><?php echo $section_name ?></option>
                                </select> 
        ...
        <div class="box box-primary" id="box_display" style="display:none">
            <div class="box-header with-border">
                <h3 class="box-title"><?php echo $this->lang->line( 'assign_subject' ); ?>
                </h3>
                <div class="box-tools pull-right">
                    <button id="btnAdd" class="btn btn-primary btn-sm checkbox-toggle pull-right" type="button">
                        <i class="fa fa-plus"></i> <?php echo $this->lang->line( 'add' ); ?></button>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    function GetDynamicTextBox( value ) {
		...
		row += '<label for="inputKey" class="col-md-1 control-label">Max Marks</label>';
        row += '<div class="col-md-1"><input type="number" name="max_marks_' + value + '" style="" class="form-control"></div>';
		row += '<label for="inputKey" class="col-md-1 control-label">Passing Marks</label>';
        row += '<div class="col-md-1"><input type="number" name="passing_marks_' + value + '" style="" class="form-control" /></div>';
		
		row += '<div class="col-md-1"><button id="btnRemove" style="" class="btn btn-sm btn-danger" type="button"><i class="fa fa-trash"></i></button></div>';
...
<script type="text/javascript">
                        ...
                        if ( data.st === 1 ) {
                            ...
                            if ( response && response.length > 0 ) {
                                for ( i = 0; i < response.length; ++i ) {
									...
									var passing_marks = response[i].passing_marks;
									var max_marks = response[i].max_marks;
									
									appendRow( subject_id, teacher_id, row_id,passing_marks, max_marks  );
                                }
                            } else {
                                appendRow( 0, 0, 0 );
                            }
    ...
    function appendRow( subject_id, teacher_id, row_id,passing_marks,max_marks ) {
       ...
       row += '<option value="<?php echo $teacher['id'] ?>" ' + selected + '><?php echo $teacher['name'] ?></option>';
        ...
        row += '<label for="inputKey" class="col-md-1 control-label">Max Marks</label>';
		row += '<div class="col-md-1"><input type="number" value="' + max_marks + '" name="max_marks_' + value + '" style="" class="form-control"></div>';
		
		row += '<label for="inputKey" class="col-md-1 control-label" >Passing Marks</label>';
		row += '<div class="col-md-1"><input type="number"  name="passing_marks_' + value + '"style="" value="' + passing_marks + '" class="form-control"/></div>';
